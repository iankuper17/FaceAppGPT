...
export function ShareCard({ analysisId, score, percentile, imageUrl }: ShareCardProps) {
  ...
  async function handleNativeShare() {
    try {
      ...
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({ title: "FaceScore", text: caption, files: [file] });
        return;
      }
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <GlassCard className="overflow-hidden">
        {/* Story preview — mirrors the generated image layout */}
        <div className="relative w-full flex justify-center p-5 pb-3">
          <div className="relative aspect-[9/16] w-44 rounded-2xl overflow-hidden bg-[#08080c] shadow-glass-lg">
            {/* Photo top half */}
            <div className="absolute inset-x-0 top-0 h-[55%]">
              ...
              {/* Top brand overlay */}
              <div className="absolute inset-x-0 top-0 h-8 bg-gradient-to-b from-[rgba(8,8,12,0.6)] to-transparent flex items-start justify-center pt-2">
                <span className="text-[6px] text-white/50 uppercase tracking-[0.15em]">
                  FaceScore
                </span>
              </div>
              {/* Bottom fade */}
              <div className="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-b from-transparent to-[#08080c]" />
            </div>

            {/* Content bottom half */}
            <div className="absolute inset-x-0 bottom-0 h-[48%] flex flex-col items-center px-3 pb-2">
              <p className="text-[26px] leading-none font-extrabold text-gradient-ig">
                {score.toFixed(1)}
              </p>
              <p className="text-[6px] text-white/35 mt-0.5 text-center">
                More attractive than {pct}%
              </p>

              <div className="w-4 h-[1px] bg-white/10 my-1.5" />

              <div className="flex flex-wrap justify-center gap-[3px]">
                {["Approachable", "Trustworthy", "Confident"].map((t) => (
                  <span
                    key={t}
              ...
              <p className="text-[6px] text-white/15 mb-0.5">
                What&apos;s your score?
              </p>
              <p className="text-[5px] font-medium text-gradient-ig">
                facescore.ai
              </p>
            </div>
          </div>
